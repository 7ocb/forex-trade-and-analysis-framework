...
class PpdeSimple(timer:Timer, config:Config, context:Context) {
  ...
  private def openTrade(direction:Direction):Unit = {
    ...
    val stopDist = config.stopDistance
    val takeProfitDist = config.takeDistance
    val delayDist = config.delayDistance
